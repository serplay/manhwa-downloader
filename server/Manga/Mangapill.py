...
class Mangapill:
    ...
    def download_chapters(ids, update_progress=None):
        """
        Download chapters from Mangapill and return the path to the ZIP archive.
        Args:
        ...
        try:
            for i, chap_id in enumerate(ids):
                ...
                response = req.get(f"{Mangapill.BASE_URL}/read", timeout=10, params={"chapterId": chap_id})
                response.raise_for_status()
                data = response.json()
                
                image_links = []
                for page in data:
                    image_links.append((page["img"], Mangapill.HEADER))
