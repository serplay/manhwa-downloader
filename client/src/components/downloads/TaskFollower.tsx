...
/**
 * Renders nothing. Follows a task's progress and saves the archive once on
 * success, whether or not the tray panel is open.
 */
export function TaskFollower({ task }: { task: TrackedTask }) {
  ...
  useEffect(() => {
    ...
    void saveFile(task.id, fileNameFor(task)).then((result) => {
      ...
      else toast.error(`Could not save ${task.comicTitle}. Use Save file in the downloads tray.`);
    });
